...
- (void)audioSession:(RTC_OBJC_TYPE(RTCAudioSession) *)session
    didChangeCanUseMic:(BOOL)canUseMic;
...
/** This property is only effective if useManualAudio is YES.
 *  Represents permission for WebRTC to access input bus of the VoIP audio unit.
 *  When the value is changed, the audio unit will be reinitialized. Defaults to YES.
 *  This property was introduced to work around an issue where iOS will request the microphone
 *  permission even if the call is one way only.
 */
@property(assign) BOOL canUseMic;

@property(assign) BOOL disableAGC;
